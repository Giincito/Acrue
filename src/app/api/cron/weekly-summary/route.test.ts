...
import { buildWeeklySummary, formatWeeklySummaryTelegram } from '@/lib/gemini/briefing'
...
vi.mock('@/lib/gemini/briefing', () => ({
  ...
  formatWeeklySummaryTelegram: vi.fn(),
}))
...
describe('weekly summary cron', () => {
  ...
  it('fails the cron when every Telegram delivery fails', async () => {
    ...
    vi.mocked(buildWeeklySummary).mockResolvedValue({} as never)
    vi.mocked(formatWeeklySummaryTelegram).mockReturnValue('summary')
    vi.mocked(sendTelegramMessage).mockResolvedValue(false)

    const response = await GET(
      new Request('http://acrue.test/api/cron/weekly-summary', {
    ...
    await expect(response.json()).resolves.toEqual({
      ...
      sent: 0,
      failed: 2,
      error: 'No se pudo enviar ningun resumen semanal.',
    })
  })
})
